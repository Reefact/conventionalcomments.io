...
# is ever pushed (see CONTRIBUTING.md).
#
...
# what `--generate-notes` used to produce — the reasoning is ADR-0017 in justdummies.io.
#
...
if [ ! -f "${notes_file}" ]; then
  echo "error: ${notes_file} does not exist — write this release's notes (see CONTRIBUTING.md) before tagging" >&2
  exit 1
fi
...
if [ -z "${notes}" ]; then
  echo "error: ${notes_file} has no '## ${tag}' section — write it (see CONTRIBUTING.md) before tagging" >&2
  exit 1
fi
