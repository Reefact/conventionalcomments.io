...
#
# Kept a near-copy of the file in Reefact/justdummies.io — same reason as tools/commit-lint
# and scripts/release-notes.sh: two Reefact repositories should not release by two rules.
#
...
#
# CI runs it in the `verify-tag` job of release.yml, before build and deploy even start — a tag
...
if [ "$(git for-each-ref "refs/tags/${tag}" --format='%(objecttype)')" != "tag" ]; then
  ...
  echo "  Delete it and tag again with -a, as CONTRIBUTING.md's command does." >&2
  exit 1
fi
