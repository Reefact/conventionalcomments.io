#!/bin/sh
# Validate a commit message against the conventionalcomments.io commit convention.
#
...
#
# This file is kept a near-copy of the one in Reefact/justdummies.io, deliberately:
# a maintainer moving between Reefact repositories should not have to remember
# which set of rules is in force, and a linter that drifts is a linter nobody
# trusts. Only the scope table differs, because the components differ.
#
# Usage:
...
# the local hook lets them through. CI (--ci) rejects them instead: a
# placeholder merged before its autosquash would otherwise land, unlinted, in
# protected history.
case "$subject" in
  'fixup! '*|'squash! '*|'amend! '*)
    if [ "$strict" = 0 ]; then
...
if [ -z "$subject" ]; then
  ...
  if printf '%s' "$subject" | grep -Eq "^(${TYPES})(\((${SCOPES})(,(${SCOPES}))*\))?!?: [a-z]"; then
    ...
    # the design tokens serves both the site and the mockups. See CONTRIBUTING.md ->
    # "Scope".
    case "$subject" in
      feat:*|feat!:*|fix:*|fix!:*)
        vtype="${subject%%[:!(]*}"
    ...
    if printf '%s' "$subject" | grep -Eq '^[a-z]+\([^)]*, '; then
      err "scopes are comma-separated with no space: '(schema,site)', not '(schema, site)'"
    fi
